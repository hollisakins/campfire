...
def shutters_to_ds9_region(
    shutters: Union[List[dict], dict],
    target_object_id: Optional[str] = None,
) -> str:
    """Serialize NIRSpec shutter geometry as a DS9 region file (fk5).

    Each shutter becomes an fk5 ``box`` sized to the standard NIRSpec
    open-area footprint (0.22" x 0.46"). Shutters are grouped by
    ``(observation, source_id)`` — the natural slitlet — with a header
    comment naming the corresponding member target (``{observation}_{source_id}``).

    Parameters
    ----------
    shutters : list of dict or dict
        Either the full ``Campfire.get_shutters()`` result or its bare
        ``shutters`` list. Each entry needs ``center_ra``, ``center_dec``,
        ``position_angle``, ``observation``, ``source_id``, ``object_id``,
        ``shutter_idx``, ``dither_id``, ``shutter_state``.
    target_object_id : str, optional
        Highlight slitlets belonging to this object in green; everything
        else is drawn grey. Stuck-closed shutters are always red dashed.
    """
    if isinstance(shutters, dict) and "shutters" in shutters:
        shutter_list = shutters["shutters"]
    else:
    ...
    # Group by (observation, source_id) — the slitlet
    groups: Dict[Tuple[str, int], List[dict]] = {}
    for s in shutter_list:
        key = (s.get("observation", ""), int(s.get("source_id", 0)))
        groups.setdefault(key, []).append(s)

    # Order slitlets: member shutters first (if a target object is named),
    # then by observation/source_id for determinism.
    def _is_member(key: Tuple[str, int]) -> bool:
        if target_object_id is None:
            return False
        return any(s.get("object_id") == target_object_id for s in groups[key])

    ordered_keys = sorted(
        groups.keys(),
        key=lambda k: (not _is_member(k), k[0], k[1]),
    )

    w_arcsec = SHUTTER_WIDTH_ARCSEC
    h_arcsec = SHUTTER_HEIGHT_ARCSEC

    lines: List[str] = [
        "# Region file format: DS9 version 4.1",
        "# Generated by CAMPFIRE (campfire.imaging.shutters_to_ds9_region)",
    ]
    if target_object_id is not None:
        lines.append(f"# Target object: {target_object_id}")
    lines.append(
        "# Shutter dimensions: "
        f"{w_arcsec}\" (dispersion) x {h_arcsec}\" (cross-dispersion)"
    )
    lines.append(
        'global font="helvetica 9 normal roman" select=1 highlite=1 '
        "edit=1 move=1 delete=1 include=1 fixed=0"
    )
    lines.append("fk5")

    for key in ordered_keys:
        obs, src = key
        target_id = f"{obs}_{src}" if obs else str(src)
        member = _is_member(key)
        group = groups[key]
        n_open = sum(1 for s in group if s.get("shutter_state") != "stuck_closed")
        marker = "(member spectrum)" if member else ""
        lines.append("")
        lines.append(
            f"# Slitlet observation={obs} source_id={src} "
            f"-> target {target_id} {marker}".rstrip()
        )
        lines.append(f"#   shutters: {len(group)} ({n_open} open)")

        for s in group:
            ra = s["center_ra"]
            dec = s["center_dec"]
            pa = s["position_angle"]
            state = s.get("shutter_state", "open")
            is_stuck = state == "stuck_closed"
            if is_stuck:
                color = "red"
                dash = " dash=1"
            elif member:
                ...
                dash = ""
            text = (
                f"dither={s.get('dither_id', 0)} "
                f"idx={s.get('shutter_idx', 0)} state={state}"
            )
            lines.append(
                f'box({ra:.7f},{dec:.7f},{w_arcsec}",{h_arcsec}",{pa:.4f})'
                f" # color={color}{dash} text={{{text}}}"
            )
