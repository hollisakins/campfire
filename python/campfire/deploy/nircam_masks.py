...
def import_masks(field, config, dry_run=False):
    ...
    require_pipeline("import-masks (needs jwst WCS + regions parsing)")
...
def mask_drift_report(field, config):
    ...
    * ``local_only`` — a non-generated ``.reg`` for an exposure with no DB mask
      (hand-drawn, never ``import-masks``'d — the portal can't see it). Never
      told to delete; it's the safety net for hand-drawn masks.
    * ``not_materialized`` — DB has polygons but no local ``.reg`` (``pull-masks``
    ...
    for p in local_only:
        print(f"  LOCAL-ONLY (not in DB, portal can't see it): {p}\n"
              f"        run: campfire deploy nircam import-masks --field {field}")
