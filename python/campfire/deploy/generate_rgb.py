"""
Generate RGB cutout images with NIRSpec slit overlays.

Extracted from scripts/plot_slits.py for integration into the deploy CLI.
"""
...
# ---------------------------------------------------------------------------
# Field-specific mosaic paths and catalogs
# ---------------------------------------------------------------------------

COSMOS_CATALOG = '/research/EMBER/target_selection/cosmos/data/cosmos_v1.2.1_positions.fits'
COSMOS_IMAGE_DATA = {
    'f115w': '/V/maurice/mosaics/cosmos/f115w/mosaic_nircam_f115w_cosmos_30mas_v0p?_{tile}_{ext}.fits',
    'f150w': '/V/maurice/mosaics/cosmos/f150w/mosaic_nircam_f150w_cosmos_30mas_v0p?_{tile}_{ext}.fits',
    'f200w': '/V/maurice/mosaics/cosmos/f200w/mosaic_nircam_f200w_cosmos_30mas_v0p?_{tile}_{ext}.fits',
    'f182m': '/V/maurice/mosaics/cosmos/f182m/mosaic_nircam_f182m_cosmos_30mas_v0p?_{tile}_{ext}.fits',
    'f210m': '/V/maurice/mosaics/cosmos/f210m/mosaic_nircam_f210m_cosmos_30mas_v0p?_{tile}_{ext}.fits',
    'f277w': '/V/maurice/mosaics/cosmos/f277w/mosaic_nircam_f277w_cosmos_30mas_v0p?_{tile}_{ext}.fits',
    'f356w': '/V/maurice/mosaics/cosmos/f356w/mosaic_nircam_f356w_cosmos_30mas_v0p?_{tile}_{ext}.fits',
    'f444w': '/V/maurice/mosaics/cosmos/f444w/mosaic_nircam_f444w_cosmos_30mas_v0p?_{tile}_{ext}.fits',
}
...
def gen_rgb_image(input_dict, noisesig=1, noiselum=0.1, satpercent=0.5, save=False):
    """Generate an RGB image from a dict of filter data and color weights."""
    ...
    rgb_total = 0
    for filt in filters:
        rgb = filter_colors[filt][:, np.newaxis, np.newaxis] * image_data_dict[filt]
        rgb_total += rgb
    r, g, b = rgb_total / rgb_lum_sum[:, np.newaxis, np.newaxis]
    ...
    for arr in [r, g, b]:
        # in-place would alias; recompute per-channel
        pass
...
def get_tile(coords):
    """
    Get the COSMOS-Web tile for a given coordinate.

    Finds the closest match to the center coordinates of the tiles.
    Returns None for coordinates outside the COSMOS-Web footprint.
    """
    single = False
    ...
    tiles = np.array(['A1', 'A2', 'A3', 'A4', 'A5', 'A6', 'A7', 'A8', 'A9', 'A10',
                       'B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B9', 'B10'])
    ra = np.array([149.83060849224526, 149.96618456679428, 150.10175329130632, 150.23731326578584,
                    150.37286289023743, 149.7647850185184, 149.9003454431693, 150.03589926778474,
                    150.17144509237895, 150.30698131694916, 149.96220693940455, 150.09747486414722,
                    150.23328673819674, 150.36919886197347, 150.50453028662352, 149.89657956583684,
                    150.03225868964083, 150.16766126474636, 150.30327548908838, 150.4387048134958])
    dec = np.array([2.2105308808257207, 2.1612435187006245, 2.111943456573772, 2.062631644446241,
                     2.013309032319107, 2.0296854374792526, 1.9804026753817368, 1.9311082632823562,
                     1.8818031511845486, 1.8324881890870612, 2.5720372673973584, 2.5230015052794137,
                     2.473525843055812, 2.423885930793452, 2.374833668674315, 2.3912872741259417,
                     2.341569861870867, 2.292723449827574, 2.2433903376662596, 2.1941086255207374])
...
def get_source_pos(spec_file):
    """Extract median source RA/Dec from the exposure table in a spec FITS file."""
    exp = Table.read(spec_file, hdu=7)
    return (np.median(exp['source_ra'][exp['source_ra'] != 0]),
            np.median(exp['source_dec'][exp['source_dec'] != 0]))
...
def fit_offsets(msa_coords, ref_coords, save_plot=None):
    """
    Fit a 2D polynomial to astrometric offsets between MSA and reference catalogs.

    Returns (dra_interp, ddec_interp) callables that map (ra, dec) -> offset in arcsec.
    If save_plot is a path, saves the QA diagnostic plot there.
    """
    ...
    if save_plot:
        scale = np.nanmax(np.sqrt(dra ** 2 + ddec ** 2)) / 20
        scale_units = 'xy'
        ...
        plt.close()
        print(f"  Saved astrometric shift QA plot: {save_plot}")

    return lra, ldec


def get_exposure_table(spec_file):
    """Load and clean the exposure table from a spec FITS file.

    Deduplicates NRS1/NRS2 rows so each dither position appears once.
    Works for all nod types (3-SHUTTER-SLITLET, NONE, etc.).
    """
    exp = Table.read(spec_file, hdu=7)
    ...
    if len(nrs1_files) == len(exp) or len(nrs2_files) == len(exp):
        # All one detector — no deduplication needed
        pass
    elif nrs1_files and nrs2_files:
        # Both detectors present — keep only NRS2 to avoid double-counting
...
def make_nircam_rgb_cutout(field, tile, coord, cutout_width=3 * u.arcsec):
    """Make an RGB cutout from NIRCam mosaics for a given field and position."""
    match field:
        case 'cosmos':
    ...
    for band in ['f115w', 'f150w', 'f182m', 'f200w', 'f210m', 'f277w', 'f356w', 'f444w']:
        ...
        if '?' in filepath:
            i = 10
            new_filepath = filepath.replace('?',f'{i}')
            while not os.path.exists(new_filepath):
                new_filepath = filepath.replace('?', f'{i-1}')
                i -= 1
                if i==1:
                    break
...
def plot_single_object(spec_files, field, output, cutout_width=3 * u.arcsec, override_pos=None):
    """Generate an RGB cutout image with NIRSpec slit overlays for a single object."""
    ...
    for t in exp:
        ...
        for i in [-1, 0, 1]:
            ...
            p = mpl.patches.Rectangle(
                ((c.ra.value - source_c.ra.value - 0.22 / 3600 / 2) * 3600 * np.cos(np.radians(source_c.dec.value)),
                 (c.dec.value - source_c.dec.value - 0.46 / 3600 / 2) * 3600),
...
def _load_ref_catalog(field):
    """Load the reference catalog for astrometric matching."""
    match field:
        case 'cosmos':
            ref_cat = fits.open(COSMOS_CATALOG)
            return SkyCoord(ref_cat[1].data['ra'], ref_cat[1].data['dec'], unit='deg')
        case 'uds':
...
def generate_rgb_images(
    ...
    *,
    ...
    """
    Generate RGB cutout images with slit overlays.

    Discovers *_spec.fits files in obs_dir, deduplicates by source ID,
    and generates {obs}_{srcid}_rgb.png for each.

    If an MSA catalog ({obs}_msacat.csv) is present, fits astrometric
    shifts automatically (equivalent to --approve-shifts in the original
    script) and saves a QA plot to {obs}_shift_qa.png.
    ...
    if not spec_files_all:
        print("No spec files found for RGB generation.")
        return 0
    ...
    if not srcids:
        print("No matching source IDs for RGB generation.")
        return 0

    # Collect positions for all sources (needed for shift fitting)
    obs_ra, obs_dec = [], []
    srcid_spec_files = {}
    for srcid in srcids:
        ...
        if not spec_files:
            continue
        srcid_spec_files[srcid] = spec_files
        ra, dec = get_source_pos(spec_files[0])
        obs_ra.append(ra)
        obs_dec.append(dec)

    # Try astrometric shift fitting
    dra_interp, ddec_interp = None, None
    msacat_file = obs_dir / f'{obs_name}_msacat.csv'
    if msacat_file.exists():
        ref_coords = _load_ref_catalog(field)
        if ref_coords is not None:
            try:
                ...
                # Filter MSA catalog to <1.5 arcmin from nearest observed object
                idx, d2d, d3d = msa_coords.match_to_catalog_sky(obs_coords)
                msa_coords = msa_coords[d2d < 1.5 * u.arcmin]

                qa_plot = obs_dir / f'{obs_name}_shift_qa.png'
                dra_interp, ddec_interp = fit_offsets(msa_coords, ref_coords, save_plot=str(qa_plot))
                print(f"  Fitted astrometric shifts using {len(msa_coords)} MSA sources")
            except Exception as e:
                print(f"  Warning: Astrometric shift fitting failed: {e}")
                dra_interp, ddec_interp = None, None
    else:
        print(f"  No MSA catalog found ({msacat_file.name}), skipping astrometric shifts")

    generated = 0
    for i, srcid in enumerate(srcids):
        output = obs_dir / f'{obs_name}_{srcid}_rgb.png'
        if output.exists() and not overwrite:
            continue

        if srcid not in srcid_spec_files:
            continue

        print(f"  Generating RGB/slit plot for {obs_name}_{srcid}")

        ra, dec = obs_ra[i], obs_dec[i]

        # Apply astrometric shifts if available
        ...
        try:
            plot_single_object(srcid_spec_files[srcid], field, output=str(output), override_pos=(ra, dec))
            generated += 1
        except Exception as e:
            print(f"  Warning: RGB generation failed for {obs_name}_{srcid}: {e}")
