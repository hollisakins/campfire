...
::
    ...
    stacked = calibrate_and_stack(prism, obj.photometry, object_id=obj.object_id)
    stacked.plot()
"""
...
def get_filter_curve(band: str) -> tuple:
    """Get a filter transmission curve (wavelength_um, transmission).

    Checks the local cache first, downloads from SVO if needed, falls
    back to a top-hat approximation using known band edges.
    """
...
def synthetic_photometry(
    ...
    denom = np.trapezoid(T_v * wavelength, wavelength)
    if denom == 0:
        return np.nan, np.nan

    synth_flux = np.trapezoid(flux_v * T_v * wavelength, wavelength) / denom
...
class CalibrationResult:
    ...
    def plot(self, axes=None):
        ...
        ax_spec.set_title(
            f"{self.original.spectrum_id}  {self.original.grating}  —  "
...
def calibrate_to_photometry(
    ...
    calibrated = SpectrumData(
        ...
        spectrum_id=spec_data.spectrum_id,
        flam=cal_flam,
...
def stack_spectra(
    ...
    combines them using inverse-variance weighting (default) or median.
    ...
    wavelength_grid : np.ndarray, optional
        Common wavelength grid (microns). Default: grid from the input
        with the most pixels.
    object_id : str, optional
        Object identifier for the stacked output's synthetic
        ``spectrum_id``. If given, the stacked spectrum's ``spectrum_id``
        is ``'stack:<object_id>:<grating>'``; otherwise it falls back to
        ``'stack:<grating>:<n>spectra'``.

    Returns
    -------
    SpectrumData
        Stacked spectrum. ``grating`` is taken from the first input.
    ...
    if object_id:
        stacked_id = f"stack:{object_id}:{grating}"
    else:
...
class StackResult:
    ...
    def plot(self, axes=None):
        ...
        ax_overlay.set_title(
            f"{self.spectrum.spectrum_id}  {self.spectrum.grating}  —  "
...
def calibrate_and_stack(
    ...
    **calibration_kwargs,
    ...
    object_id : str, optional
        Passed through to :func:`stack_spectra` to compose the stacked
        ``spectrum_id``.
    **calibration_kwargs
