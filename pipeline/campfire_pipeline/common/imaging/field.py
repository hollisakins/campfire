"""
ImagingField: generic dataclass + workspace logic for imaging-arm fields.

Per-instrument subclasses (``NircamField``, ``MiriField``) set the class
attributes ``INSTRUMENT_NAME``, ``DETECTOR_SHAPE``, ``KNOWN_STEPS``, and
override ``_setup_instrument_workspace()`` if they need instrument-specific
reference subdirectories beyond the generic refcat/masks/flats trio.

Raw uncal layout is PID-organized:
``$CAMPFIRE_ROOT/raw/<instrument>/{PID}/{filter}/``. PIDs are derived from
the leading ``jwNNNNN`` of each ``files`` glob, so a field that spans
multiple programs naturally pulls from multiple PID directories.
"""
...
def _extract_pid(pattern):
    ...
    layout under ``$CAMPFIRE_ROOT/raw/<instrument>/`` uses the unpadded
    integer form, so leading zeros are stripped.
    """
...
@dataclass
class ImagingField:
    """Generic imaging-arm field: filters, file globs, tile WCS, workspace."""

    # Subclass overrides — set on the subclass via ClassVar so the dataclass
    # decorator doesn't treat them as instance fields.
    INSTRUMENT_NAME: ClassVar[str] = 'imaging'
    DETECTOR_SHAPE: ClassVar[tuple] = (2048, 2048)
    KNOWN_STEPS: ClassVar[frozenset] = frozenset()
    ...
    step_overrides: dict = dataclass_field(default_factory=dict)
    skip: List[str] = dataclass_field(default_factory=list)
    rgb: Optional[dict] = None  # optional [field.rgb] block
    ...
    wcs_shift_rules: List[dict] = dataclass_field(default_factory=list)

    # Populated by setup_workspace()
    campfire_root: Optional[str] = None
    raw_root: Optional[str] = None  # $CAMPFIRE_ROOT/raw/<instrument> (parent of PID dirs)
    products_dir: Optional[str] = None
    reference_dir: Optional[str] = None

    # Generic reference subdirectories — present for every imaging arm.
    ...
    def load(cls, name, fields_file=None):
        ...
        Returns an instance of ``cls`` (i.e. the appropriate per-instrument
        subclass when called as ``NircamField.load(...)`` /
        ``MiriField.load(...)``).
        ...
        fields_file : str, optional
            ...
            ``config.resolve_fields_file()`` search order.
        """
        ...
        known_steps = cls.KNOWN_STEPS
        tiles = {}
        reserved_keys = (
            {'filters', 'files', 'skip', 'tangent_point', 'rgb'} | set(known_steps)
        )
        ...
        # Optional [<field>.rgb] block. Validation of channel filters against
        # `filters` happens at use-site so a stale RGB block doesn't block
        # other commands.
        rgb_cfg = fc.get('rgb')
    ...
    def setup_workspace(self, campfire_root=None):
        ...
        self.raw_root = os.path.join(campfire_root, 'raw', instrument)
        self.products_dir = os.path.join(campfire_root, 'products', instrument, self.name)
        self.reference_dir = os.path.join(campfire_root, 'reference', instrument, self.name)

        # Generic reference subdirectories (every imaging arm needs these).
        ...
        for d in [self.refcat_dir, self.mask_dir, self.flats_dir]:
            os.makedirs(d, exist_ok=True)

        # Subclass hook for instrument-specific reference subdirectories
        # (NircamField → bad_pixels, wisps; MiriField → warm_pixels, etc.).
        self._setup_instrument_workspace()

        log(f"Workspace ready for field '{self.name}' at {self.products_dir}")

    def _setup_instrument_workspace(self):
        """Hook for subclasses to create instrument-specific reference subdirs.

        Called from ``setup_workspace()`` after the generic dirs are created.
    ...
    def filter_dir(self, filter_name):
        ...
        ``$CAMPFIRE_ROOT/products/<instrument>/<field>/<filter>/`` holds every
    ...
    def get_uncal_files(self, filter_name, skip=None):
        ...
        Globs across ``$CAMPFIRE_ROOT/raw/<instrument>/{PID}/{filter}/`` for
        every PID derived from this field's ``files`` patterns. The
        field-wide ``skip`` list (from fields.toml) is always applied;
        caller-passed ``skip`` adds to it.
        """
    ...
    def get_exposure_files(self, filter_name, skip=None, with_step=None,
        ...
        These are the files that the pipeline mutates in place — one FITS
        ...
        status : StepStatus, optional
            ...
            in the orchestrator (which marks fresh CFP keys onto the cache
            as steps finish); other callers can omit it and pay the
            per-file fits.open.
        ...
        if with_step is not None:
            if status is not None:
                ...
                # has_step is instrument-agnostic (just checks header presence).
