import os, sys, tqdm, pickle, time
import psutil
from time import sleep
import numpy as np

from htools.utils.filters import Filters
...
from htools.plotting import set_style
set_style('sans')
...
def compute_ranges_from_wc(wc,nom):
    ranges = []
    # NRS1 coverage
    if wc[1] != -1:  # NRS1 has coverage (not -1)
        if wc[0] == -1 and wc[1] == -2:  # Entire spectrum on NRS1
            ranges.append((nom[0], nom[1]))
        elif wc[1] == -2:  # Extends past right edge
            ranges.append((wc[0], nom[1]))
        else:  # Ends at specific wavelength
            ranges.append((wc[0] if wc[0] > 0 else nom[0], wc[1]))
    
    # NRS2 coverage  
    if wc[2] != -2:  # NRS2 has coverage (not -2)
        if wc[2] == -1 and wc[3] == -2:  # Entire spectrum on NRS2
            ranges.append((nom[0], nom[1]))
        elif wc[2] == -1:  # Starts from beginning
            ranges.append((nom[0], wc[3]))
        elif wc[3] == -2:  # Extends to end
...
class InspectionPlotGenerator:
    ...
    # def filter_ids_on_existing_files(self, IDs):
    #     N = 0

    #     IDs_new = []
    #     for ID in IDs:
    #         outfilename = f"cosmos_web_sed_{ID}.pdf"
    ...
    #     if self.verbose:
    #         log(f'{N}/{len(IDs)} completed so far ({N/len(IDs)*100:.1f}%)')
    ...
    def make_nircam_rgb_cutout(self, tile, coord, cutout_width):
        ...
        for band in ['f115w','f150w','f182m','f200w','f210m','f277w','f356w','f444w']:
            if not band in self.image_data: 
                return None,None
    ...
    def make_single_object_plot(self, ID, 
        ...
        if os.path.exists(outfile) and not overwrite:
            print(f"Plot file for {ID} already exists, skipping...")
            return
        ...
        jwst_bands = ['f090w','f115w','f140m','f150w','f162m','f182m','f200w','f210m','f250m','f277w','f300m','f335m','f356w','f360m','f410m','f430m','f444w','f480m','f560w','f770w','f1000w','f1280w','f1500w','f1800w','f2100w']
        jwst_axes = [plt.subplot(gs[8:12,i:i+4]) for i in range(0,63,4)] + [plt.subplot(gs[12:16,i:i+4]) for i in range(0,36,4)]

        ax_flags = plt.subplot(gs[12:16,36:44])
        ax_flags.axis('off')
        ax_flags.annotate(f"kron_corr: {cat['kron_corr']:.2f}", (0.025, 0.95), color='k', ha='left', va='top', xycoords='axes fraction', weight='bold', fontsize=7)
        # ax_flags.annotate(f"aper_corr (0.2''): {cat['aper_corr'][2]:.2f}", (0.025, 0.80), color='k', ha='left', va='top', xycoords='axes fraction', weight='bold', fontsize=7)
        # ax_flags.annotate(f"peak SB: {cat['peak_surface_brightness']:.2f}", (0.025, 0.65), color='k', ha='left', va='top', xycoords='axes fraction', weight='bold', fontsize=7)
        # ax_flags.annotate(f"use_circ: {bool(cat['use_circ'])}, {np.sqrt(cat['kron1_a']*cat['kron1_b']):.2f}", (0.025, 0.75), color='k', ha='left', va='top', xycoords='axes fraction', weight='bold', fontsize=7)
        # idx = np.where(self.catalog['id']==ID)[0][0]
        # flag_file = '/research/COSMOS-3D/highz/data/catalog_cosmos_v1.0_highz_color_selection_neighor_flags_15x100.npy'
        # flag_neighbor = np.load(flag_file)

        # TODO update neighbor flags! compute some sort of MSATA isolation metric using the segmaps + detection images
        # bn10_10 = cat['neighbor_ratio_10pix'] >= 10.
        # bn20_100 = cat['neighbor_ratio_20pix'] >= 100.
        # bn30_1000 = cat['neighbor_ratio_30pix'] >= 1000.
        # if bn10_10 or bn20_100 or bn30_1000:
        #     flagstr = 'BN: '
        #     if bn10_10: flagstr += '10x10 '
        #     if bn20_100: flagstr += '20x100 '
        #     if bn30_1000: flagstr += '30x1000'
        #     ax_flags.annotate(flagstr, (0.05, 0.0), color='r', ha='left', va='bottom', xycoords='axes fraction', weight='bold', fontsize=7)
        ...
        for ax, name in zip(ground_axes, ground_bands):
            if name=='Ks': name = 'K_s'
            ax.text(0.05, 0.95, f'${name}$', transform=ax.transAxes, color='k', va='top', ha='left', path_effects=[pe.withStroke(linewidth=1.3, foreground='w')], size=8)

        for ax, name in zip(hst_axes, hst_bands):
            ax.text(0.05, 0.95, f'{name.upper()}', transform=ax.transAxes, color='k', va='top', ha='left', path_effects=[pe.withStroke(linewidth=1.3, foreground='w')], size=8)

        for ax, name in zip(jwst_axes, jwst_bands):
            ax.text(0.05, 0.95, f'{name.upper()}', transform=ax.transAxes, color='k', va='top', ha='left', path_effects=[pe.withStroke(linewidth=1.3, foreground='w')], size=8)
        
        if msata:
            ax_detec.text(0.04, 0.96, r'MSATA',  transform=ax_detec.transAxes, color='white', va='top', ha='left', path_effects=[pe.withStroke(linewidth=2, foreground='k')], size=10)
        else:
            ax_detec.text(0.04, 0.96, r'Detection',  transform=ax_detec.transAxes, color='white', va='top', ha='left', path_effects=[pe.withStroke(linewidth=2, foreground='k')], size=10)
        ax_segm.text(0.04, 0.96, r'Segmentation',  transform=ax_segm.transAxes, color='white', va='top', ha='left', path_effects=[pe.withStroke(linewidth=2, foreground='k')], size=10)
        ax_rgb.text(0.04, 0.96, r'NIRCam RGB',  transform=ax_rgb.transAxes, color='white', va='top', ha='left', path_effects=[pe.withStroke(linewidth=2, foreground='k')], size=10)
        ...
        if self.verbose: log('Making detection cutout...')
        ...
        if msata:
            ...
            cmap.set_bad('k')
            vmin = -26.5
            vmax = np.nanmax(sci[cen-5:cen+5,cen-5:cen+5])
            if vmax < vmin:
                vmin = vmax - 1
            ax_detec.imshow(sci, extent=extent, vmin=vmin, vmax=vmax, cmap=cmap, interpolation='nearest')#, transform=tr + ax_detc.transData)
            ...
            if vmax < 12: 
                vmax = 12
            ax_detec.imshow(d, extent=extent, vmin=1, vmax=vmax, cmap='Greys_r', interpolation='nearest')#, transform=tr + ax_detc.transData)
        ...
        if not msata:
            ...
            ax_detec.add_patch(mpl.patches.Ellipse((0,0), width=2*a, height=2*b, angle=theta, facecolor='none', edgecolor='salmon', linestyle='--', linewidth=0.5))
            ax_detec.add_patch(mpl.patches.Circle((0,0), radius=0.1, facecolor='none', edgecolor='w', linestyle='-.', linewidth=0.5))
            # ax_detec.add_patch(mpl.patches.Circle((0,0), radius=0.25, facecolor='none', edgecolor='w', linestyle=':', linewidth=0.5))
            # a, b, theta = cat['kron2_a'], cat['kron2_b'], np.degrees(cat['theta'])
            # ax_detec.add_patch(mpl.patches.Ellipse((0,0), width=2*a, height=2*b, angle=theta, facecolor='none', edgecolor='lightblue', linestyle='--', linewidth=0.5))
        ...
        ax_segm.imshow(d, extent=extent, cmap=segm_cmap, interpolation='none') #transform=tr + ax_segm.transData)
        ...
        size = np.shape(imrgb)[0]
        w = 100
        plt.imsave(f'/Users/hba423/Downloads/cosmos_{ID}.png', imrgb[size//2-w//2:size//2+w//2,size//2-w//2:size//2+w//2,:])

        if imrgb is not None:
            ax_rgb.imshow(imrgb, extent=extent)
        else:
            imrgb = np.zeros((100,100,3))
            ax_rgb.imshow(imrgb, extent=[-cutout_width.to(u.arcsec).value/2, cutout_width.to(u.arcsec).value/2, -cutout_width.to(u.arcsec).value/2, cutout_width.to(u.arcsec).value/2])
        ...
        short_names = ['vis','f435w','f606w','f814w','f098m','f090w','f115w','f140m','f150w','f182m','f200w','f210m','f250m','f277w','f335m','f356w','f360m','f410m','f430m','f444w','f460m','f480m','f770w']
        filters = Filters(short_names)
        ...
        min_mag, max_mag, handle = self.plot_data(ax_sed, wav, wav_min, wav_max, flux, error, colors, sizes, zorders, annotate=False, plot_xerr=True, label=True)
        ...
        _, _, handle = self.plot_data(ax_sed, wav, wav_min, wav_max, flux, error, colors, sizes, zorders, marker='s', mew=0.7, annotate=False, plot_xerr=True, label=True)
        handles.append(handle)
        ...
        if self.lazy_runs:
            ...
            for key,lazy_run in self.lazy_runs.items():    
                ...
                if 'fiducial' in key:
                    zorder=4
                p, = ax_sed.plot(lam_obs, y, linewidth=1, color=color, label=label, zorder=zorder)
                ...
                handles.append(p)
                labels.append(label + r' ($z =' f'{zmed:.1f}' r'^{+' f'{zuperr:.1f}' r'}_{-' f'{zloerr:.1f}' r'}$)')

                if file_lowz:
                    handles.append(p_lowz)
                    labels.append(label + ' (forced $z<7$)')
        ...
        N = len(snr_table)
        table_positions = [0.59,0.52,0.45,0.38,0.31,0.24,0.17,0.10,0.03]
        if len(snr_table) > len(table_positions):
            snr_table = {k:v for i, (k, v) in enumerate(snr_table.items()) if i < len(table_positions)}
        i= 0
        for key, value in snr_table.items():
            pos = table_positions[i]
            ax_table.annotate(key, (0.0, pos), ha='left', va='top', xycoords='axes fraction')
            ax_table.annotate(str(value[0]) if np.isfinite(value[0]) else '', (0.26, pos), ha='center', va='top', color=map_snr_color(value[0]), xycoords='axes fraction')
            ax_table.annotate(str(value[1]) if np.isfinite(value[1]) else '', (0.42, pos), ha='center', va='top', color=map_snr_color(value[1]), xycoords='axes fraction')
            ax_table.annotate(str(value[2]) if np.isfinite(value[2]) else '', (0.58, pos), ha='center', va='top', color=map_snr_color(value[2]), xycoords='axes fraction')
            ax_table.annotate(str(value[3]) if np.isfinite(value[3]) else '', (0.74, pos), ha='center', va='top', color=map_snr_color(value[3]), xycoords='axes fraction')
            ax_table.annotate(str(value[4]) if np.isfinite(value[3]) else '', (0.90, pos), ha='center', va='top', color=map_snr_color(value[3]), xycoords='axes fraction')
            if i%2!=0:
                ax_table.fill_between([0, 1], pos-0.055, pos+0.015, edgecolor='none', facecolor='0.9', zorder=-100)
            i += 1

        if len(handles) > 0:
            leg = ax_sed.legend(handles, labels, loc='upper left', frameon=False, fontsize=9)
        ...
        if prism_wavelength_coverage is not None:
            if all([wc==0 for wc in prism_wavelength_coverage]):
                ...
                for r in ranges:
                    ax_sed.fill_between(r, ymax-0.05*(ymax-ymin), ymax-0.03*(ymax-ymin), edgecolor='none', facecolor='b', zorder=1000, alpha=0.2)
        ...
        if g395m_wavelength_coverage is not None:
            if all([wc==0 for wc in g395m_wavelength_coverage]):
                ...
                for r in ranges:
                    ax_sed.fill_between(r, ymax-0.03*(ymax-ymin), ymax-0.01*(ymax-ymin), edgecolor='none', facecolor='r', zorder=1000, alpha=0.2)

        if prism_shutter is not None:
            ra = prism_shutter[0]
            dec = prism_shutter[1]
            shutter_c = SkyCoord(ra, dec, unit='deg')
            pa = prism_shutter[2]*u.deg

            for i in [-1,0,1]:
                c = shutter_c.directional_offset_by(-pa, i*0.53*u.arcsec)
                p = mpl.patches.Rectangle(
                    ((c.ra.value-coord.ra.value-0.22/3600/2)*3600, (c.dec.value-coord.dec.value-0.46/3600/2)*3600), 
        ...
        # import matplotlib as mpl
        # from astropy.stats import sigma_clipped_stats
        # f444w = fits.open('/Users/hba423/Downloads/primeruds_nrc_f444w_sci.fits')
        ...
        #     for file in msafiles:
        #         f = Table.read(file)
        #         if not ID in f['ID']:
        #             continue

        #         fi = f[f['ID']==ID]
        #         source_c = SkyCoord(fi['Source RA (Degrees)'][0], fi['Source Dec (Degrees)'][0], unit='deg')
        #         pa = np.mean(fi['Aperture PA (Degrees)'])*u.deg
        #         dx = (fi['Offset (x)'][0]-0.5)*0.27*u.arcsec
        #         dy = (fi['Offset (y)'][0]-0.5)*0.53*u.arcsec
        ...
        #     cutout = Cutout2D(f444w, position=source_c, size=7*u.arcsec, wcs=wcs)
        #     size = np.shape(cutout.data)[0]

        #     std = sigma_clipped_stats(cutout.data)[2]
        #     plt.imshow(cutout.data/std, vmin=-3, vmax=10, 
        #         cmap = 'Greys',
        #         extent=[-size*ps/2,size*ps/2,-size*ps/2,size*ps/2], 
        #         zorder=0)
        #     # ax.scatter(source_c.ra, source_c.dec)
        ...
        #     ax.set_aspect('equal')
        #     ax.set_title(ID)
        #     plt.savefig(f'plots/{ID}_cutout.pdf')
        #     plt.close()
...
if __name__ == '__main__':
    ...
    import tqdm
    from plot_slits import get_source_pos
    ra, dec = [], []
    ...
    pos = Table.read('/research/EMBER/target_selection/cosmos/data/cosmos_v1.1_positions.ecsv')
    coords = SkyCoord(pos['ra'], pos['dec'], unit='deg')
    for i in tqdm.tqdm(range(len(object_ids))):
        c = SkyCoord(ra[i], dec[i], unit='deg')
        sep = coords.separation(c).to('arcsec').value
        idx = np.argmin(sep)
        print(object_ids[i], sep[idx])

        if sep[idx] < 0.2:
            ID = pos['id'][idx]
            generator.make_single_object_plot(ID, overwrite=args.overwrite, notes=object_ids[i])

            if os.path.exists(f'products/{obs}/cosmos_sed_{ID}.pdf'):
                os.rename(f'products/{obs}/cosmos_sed_{ID}.pdf', f'products/{obs}/{object_ids[i]}_sed.pdf')
