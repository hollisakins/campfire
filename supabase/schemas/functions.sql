...
-- via a window count. Semantics mirror the route, with two 2026-08 fixes:
--   * comments now LEFT JOIN targets/objects, so object-parented comments
--     (comments.object_id) appear in the feed instead of being silently
--     dropped by the old inner join;
...
--     and every row now carries subject_kind ('target'|'object'|'spectrum')
--     so the client can build a link that resolves — the old single
--     /nirspec/targets/<display_id> href 404'd for object- and spectrum-scoped
--     rows, i.e. essentially all post-Phase-D inspection activity;
...
-- =============================================================================
-- Three consolidated read models behind the /admin landing page, replacing a
-- dozen client round trips (several of them full-table client-side rollups).
-- All three are SECURITY DEFINER with an explicit is_admin() gate: the science
-- tables they aggregate (objects, spectra, flag_audit_log, comments) carry
-- program-overlap RLS, not admin RLS, so an invoker-rights aggregate would
-- return a plausible wrong number for an admin whose grants are partial —
-- and objects' `programs && accessible_program_slugs()` overlap test is false
-- for the empty programs[] carried by reconciliation-orphaned rows, exactly
-- the rows the dashboard wants to count. Split into three functions (not one
-- mega-RPC) so one failure cannot blank the whole page.
