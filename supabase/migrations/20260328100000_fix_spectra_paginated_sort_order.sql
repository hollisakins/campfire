-- Fix: get_filtered_spectra_paginated jsonb_agg did not preserve sort order.
-- The LEFT JOIN programs after the page_rows CTE destroys row ordering,
-- and jsonb_agg without ORDER BY produces non-deterministic element order.
-- Fix: assign row_number() in page_rows and ORDER BY it in the aggregate.
-- (Same pattern used by get_filtered_targets_paginated.)
