...
alter table "public"."spectra" add column "updated_at" timestamp with time zone not null default now();

CREATE INDEX idx_spectra_updated_at ON public.spectra USING btree (updated_at);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.bump_spectra_updated_at()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.get_spectra_for_sync(p_program_slugs text[], p_user_id uuid DEFAULT NULL::uuid, p_updated_since timestamp with time zone DEFAULT NULL::timestamp with time zone, p_limit integer DEFAULT 1000, p_offset integer DEFAULT 0)
...
CREATE OR REPLACE FUNCTION public.get_observation_manifest(p_obs_name text, p_program_slugs text[])
 RETURNS TABLE(spectra_id integer, spectrum_id text, target_id text, grating text, fits_path text, file_hash text, file_size bigint, signal_to_noise double precision, reduction_version text)
 LANGUAGE plpgsql
 STABLE
AS $function$
BEGIN
...
CREATE TRIGGER bump_spectra_updated_at_trigger BEFORE UPDATE ON public.spectra FOR EACH ROW EXECUTE FUNCTION public.bump_spectra_updated_at();

CREATE TRIGGER bump_object_version_trigger BEFORE UPDATE OF redshift_inspected, redshift_quality ON public.objects FOR EACH ROW EXECUTE FUNCTION public.bump_object_version();
