...
--   * object_band_values(p_band, mag_min, mag_max, snr_min, snr_max,
--     p_program_slugs, p_include_unpublished), NEW: SECURITY DEFINER, returns
--     (object_id, mag, snr) for every caller-visible object in the window,
--     min(mag) / max(snr) over the object's cross-matches in the band. Scope is
--     the caller's explicit slug array + unpublished opt-in (the arguments
--     every catalog RPC already applies to its own objects predicate, and the
--     only gate for the service-role client the /api/v1 bearer routes use,
--     which carries no auth.uid()), plus a verbatim copy of the
--     select_objects_by_access predicate whenever there IS a user identity
--     (defense in depth for a direct call). ONE hash join to objects instead
--     of the per-row policy chain: 10-50x cheaper (70 ms for the filter,
--     105 ms for the values of every visible object at production scale).
--   * objects_matching_band_filter: gains the same two scope parameters (the
--     old 5-argument signature is dropped) and is now the id projection of
...
--     DROP of an RPC: they never go missing).
--
...
-- The 5-argument filter signature from 20260919120000 (replaced below by the
-- 7-argument one; the RPCs that called it are re-created in the same
-- transaction). The 5-argument object_band_values only ever existed on this
-- PR's preview branch, from an earlier revision of this file.
DROP FUNCTION IF EXISTS public.objects_matching_band_filter(TEXT, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION);
DROP FUNCTION IF EXISTS public.object_band_values(TEXT, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION);
