-- Photometry band filter: SECURITY DEFINER helper + set-based band values.
--
-- The band filter shipped in 20260919120000 timed out in production
-- ("canceling statement due to statement timeout" on the NIRSpec table). Not
-- the jsonb: the bands are unnested into object_photometry_bands at write
-- time. The cost was Row Level Security evaluated per scanned row: the
-- derived table's SELECT policy is a correlated EXISTS on the parent
-- cross-match, whose own policy is a correlated EXISTS on objects, so
-- objects_matching_band_filter() ran ~30k nested subplans per call (0.6-3.5 s
-- on a 990k-row copy), and sorting on the band added one object_band_mag() /
-- object_band_snr() probe per candidate row under the same chain (2.9 s
-- more). Together with the count query that is the 8 s authenticator
-- statement_timeout on shared compute.
--
-- Fix (supabase/schemas/functions.sql, AGENTS.md):
--   * object_band_values(p_band, mag_min, mag_max, snr_min, snr_max), NEW:
--     SECURITY DEFINER, returns (object_id, mag, snr) for every viewer-visible
--     object in the window, min(mag) / max(snr) over the object's cross-matches
--     in the band. It enforces the select_objects_by_access predicate inline
--     with ONE hash join to objects instead of the per-row policy chain
--     (10-50x cheaper, measured 70 ms for the filter and 105 ms for the values
--     of every visible object at production scale).
--   * objects_matching_band_filter: same signature, now the id projection of
--     object_band_values (SECURITY INVOKER, inlinable).
--   * object_band_mag / object_band_snr: DROPPED. The six filter-contract RPCs
--     (get_filtered_spectra_paginated, get_filtered_objects_paginated,
--     get_filtered_object_ids, get_adjacent_objects, get_csv_export_spectra,
--     get_csv_export_objects) LEFT JOIN object_band_values(...) once per
--     statement for the band_mag / band_snr columns and sort keys instead of
--     calling the probes per row. Signatures and return types are unchanged,
--     so the web client needs no change and CREATE OR REPLACE suffices (no
--     DROP: the functions never go missing).
--
-- Hand-authored: no local Docker for `supabase db diff`. Every definition
-- below is copied verbatim from supabase/schemas/functions.sql (byte-identical
-- blocks, extracted mechanically at authoring time). Verified
-- to apply on a stock PostgreSQL 16 carrying the full migration chain
-- (20260328200000 .. 20260919120000) + supabase/seed.sql with the Supabase
-- auth/role pieces stubbed, followed by every supabase/tests/check_*.sql guard
-- including the new check_band_filter_scoping.sql; CI's `Supabase schema
-- guards` workflow repeats that on the real local stack.

-- =============================================================================
-- 1. Retire the per-row probes (their callers are re-created below)
-- =============================================================================

DROP FUNCTION IF EXISTS public.object_band_mag(INTEGER, TEXT);
DROP FUNCTION IF EXISTS public.object_band_snr(INTEGER, TEXT);


-- =============================================================================
-- 2. Helpers (functions.sql, verbatim)
-- =============================================================================
...
-- object_band_values: for catalog band p_band, one row per viewer-visible
...
-- enforcing the SAME viewer scope with one hash join to objects is 10-50x
-- cheaper (70 ms for the filter, 105 ms for the values of every visible
-- object). The scope predicate below is a verbatim copy of
-- select_objects_by_access (policies.sql); keep the two in step: it is what
-- makes a direct call from a viewer return only what that viewer could read
-- through the policy. A row is admitted only if its OBJECT is visible; the
-- cross-match carries no access rule of its own (photometry is an external
-- positional catalog, not a program product), and the objects join is on the
-- object primary key, so a cross-match whose object_id is still NULL (before
-- the post-rebuild FK refresh) is absent, as it is from every viewer's
-- photometry anyway. Set search_path is the usual SECURITY DEFINER hygiene.
--
...
CREATE OR REPLACE FUNCTION public.object_band_values(
  p_band TEXT,
  p_mag_min DOUBLE PRECISION DEFAULT NULL,
  p_mag_max DOUBLE PRECISION DEFAULT NULL,
  p_snr_min DOUBLE PRECISION DEFAULT NULL,
  p_snr_max DOUBLE PRECISION DEFAULT NULL
)
...
AS $$
  ...
  WHERE p_band IS NOT NULL
    AND b.band = p_band
    -- Viewer scope: verbatim select_objects_by_access (policies.sql).
...
GRANT EXECUTE ON FUNCTION public.object_band_values(TEXT, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION) TO authenticated;
GRANT EXECUTE ON FUNCTION public.object_band_values(TEXT, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION) TO service_role;
...
-- never widen it; the viewer scope inside object_band_values makes a direct
-- call equally safe. SECURITY INVOKER and a single SELECT, so it inlines.
...
AS $$
  SELECT v.object_id
  FROM public.object_band_values(p_band, p_mag_min, p_mag_max, p_snr_min, p_snr_max) v;
$$;
...
-- =============================================================================
-- 3. Filter-contract RPCs (functions.sql, verbatim; unchanged signatures)
-- =============================================================================
...
BEGIN
  ...
  WITH filtered_spectra AS NOT MATERIALIZED (
    ...
    -- band values (Mag / Band S/N columns + sort keys): one viewer-scoped scan per statement, hash-joined; NULLs when no band filter is active
    LEFT JOIN public.object_band_values(p_band, p_band_mag_min, p_band_mag_max, p_band_snr_min, p_band_snr_max) bv ON bv.object_id = t.object_id
    WHERE
...
BEGIN
  ...
  WITH candidates AS (
    ...
    FROM objects o
    -- band values (Mag / Band S/N columns + sort keys): one viewer-scoped scan per statement, hash-joined; NULLs when no band filter is active
    LEFT JOIN public.object_band_values(p_band, p_band_mag_min, p_band_mag_max, p_band_snr_min, p_band_snr_max) bv ON bv.object_id = o.id
    WHERE
      o.programs && v_filtered_program_slugs
      AND o.is_active = true
      AND (p_include_unpublished OR o.has_published_spectrum)
...
BEGIN
  ...
  -- band values (Mag / Band S/N columns + sort keys): one viewer-scoped scan per statement, hash-joined; NULLs when no band filter is active
  LEFT JOIN public.object_band_values(p_band, p_band_mag_min, p_band_mag_max, p_band_snr_min, p_band_snr_max) bv ON bv.object_id = po.id
