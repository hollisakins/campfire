-- Fix sorting bug in paginated path of get_filtered_object_ids.
--
-- The paginated path used ROW_NUMBER() OVER () (no ORDER BY), which assigned
-- arbitrary row numbers. The wrapper get_filtered_objects_paginated aggregates
-- results with ORDER BY row_num, so the scrambled row numbers produced
-- scrambled sort order. This adds the correct ORDER BY to the window function.
