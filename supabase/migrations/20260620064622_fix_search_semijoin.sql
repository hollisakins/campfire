-- Fix: the bare `search_text ILIKE '%'||p_search||'%'` predicate (PR #188) let the
-- planner mis-estimate ILIKE selectivity and fall back to walking objects_object_id_key
-- under ORDER BY object_id + LIMIT (the original 9.6s full-scan trap). Wrap search as a
-- semijoin `id IN (SELECT id FROM <table> WHERE search_text ILIKE ...)` so the candidate
-- set is computed first; the ordered-scan gamble is no longer in the plan space.
-- Validated on prod (read-only): 9.6s -> 88ms selective, 433ms broad (1273 matches).
-- Hand-trimmed from `supabase db diff` to drop spurious view/MV/accessible churn.
