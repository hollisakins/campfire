-- Fix: with a program filter active, the objects table scoped each row's
-- displayed programs / # observations / gratings aggregates to only the
-- selected program(s). The filter should choose which rows appear (objects
-- whose programs *include* a selected program — unchanged overlap test), while
-- per-row aggregates and member_targets stay scoped to the viewer's full
-- accessible program set (p_program_slugs), not the filter-narrowed array.
-- Hand-authored (CREATE OR REPLACE, signature unchanged): no local Docker for
-- `supabase db diff`. Matches supabase/schemas/functions.sql.
