-- Codex review follow-up (PR #410): gate member_targets on
-- has_published_spectrum, matching object_scoped_aggregates. The RPC is
-- reached via the service-role client (/api/v1/objects) which bypasses RLS,
-- so without this predicate draft-only targets in accessible programs leaked
-- target_id / observation / redshift_auto when p_include_unpublished = false.
-- Pre-existing gap; the previous migration widened it to filtered queries.
