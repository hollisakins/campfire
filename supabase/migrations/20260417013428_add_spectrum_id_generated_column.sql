...
alter table "public"."spectra" add column "spectrum_id" text generated always as (regexp_replace(regexp_replace(fits_path, '^.*/'::text, ''::text), '_spec\.fits$'::text, ''::text, 'i'::text)) stored;

CREATE UNIQUE INDEX idx_spectra_spectrum_id ON public.spectra USING btree (spectrum_id);

CREATE INDEX idx_spectra_spectrum_id_trgm ON public.spectra USING gin (spectrum_id public.gin_trgm_ops);
