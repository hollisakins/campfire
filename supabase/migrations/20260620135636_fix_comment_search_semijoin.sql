-- Fix: comment search (p_comment_search) timed out for broad-access users.
-- The predicate was a CORRELATED `EXISTS (SELECT 1 FROM comments c WHERE ...
-- c.object_id = o.id OR c.target_id IN (SELECT t.id FROM targets WHERE t.object_id = o.id))`
-- sitting inside an OR. Postgres can't pull a correlated EXISTS-inside-OR up into
-- a semijoin, so it re-executed the per-object targets subquery for every
-- (object x matching-comment) pair: ~34k objects x N matching comments. Prod
-- repro (authenticated admin, real RPC): broad term 'e' = 32,189 ms / 518k buffers.
--
-- Rewrite each occurrence as an UNCORRELATED semijoin so the matching id set is
-- built ONCE (object-level comments directly + target-level comments mapped to
-- their parent object via a UNION), then `o.id IN (...)` / `t.id IN (...)`.
-- Same prod call after the rewrite: 164 ms / 19k buffers (~197x). Result set
-- verified identical to the correlated form (COUNT match on a broad term).
-- Mirrors the PR #189 search_text semijoin fix; same root-cause family.
--
-- Touches: get_filtered_spectra_paginated, get_filtered_objects_paginated (count
-- + page), get_filtered_object_ids, get_adjacent_objects, get_csv_export_spectra,
-- get_csv_export_objects. Hand-trimmed from `supabase db diff` to drop spurious
-- materialized-view / view / accessible_program_slugs churn (migra false positives).
