...
-- two-probe no-unpublished-member guard (an unpublished-row probe for callers
-- who see every row, and a visible-count-vs-stored-n_spectra probe for
-- callers whose RLS hides unpublished rows) so the DISPLAYED aggregates agree
-- with the scoped filter under every caller role.
--
