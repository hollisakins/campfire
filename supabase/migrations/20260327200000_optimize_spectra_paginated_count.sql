-- Optimize get_filtered_spectra_paginated: replace COUNT(*) OVER () window
-- function with a separate count query. The window function forces PostgreSQL
-- to materialize the entire result set before LIMIT can take effect, which
-- is especially costly when the spectra join multiplies the row count.
--
-- Instead, compute total_count once with a subquery on the distance_filtered
-- CTE, and sort+paginate the page rows independently.
