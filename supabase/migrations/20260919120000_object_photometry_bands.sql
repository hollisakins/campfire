-- Photometry band catalog: an indexed, normalized mirror of the
-- object_photometry.photometry -> 'bands' jsonb so the portal can FILTER and
-- SORT on a band's AB magnitude or S/N ("everything brighter than F444W = 27
-- at S/N > 5"), plus the band-filter parameters on every RPC that shares the
-- catalog filter contract. The photometry counterpart of the spectrum_lines
-- migration (20260910040000), and deliberately built the same way.
--
...
--   tables.sql    -- object_photometry_bands (+ PK, FK to object_photometry,
--                    grants, comments) and the object_photometry.photometry
--                    column comment
--   triggers.sql  -- sync_object_photometry_bands (+ trigger on object_photometry)
--   indexes.sql   -- idx_object_photometry_bands_band_mag / _band_snr
--   policies.sql  -- RLS: select follows the parent cross-match row; admin writes
--   functions.sql -- objects_matching_band_filter, object_band_mag,
--                    object_band_snr; and the six filter-contract RPCs
--                    re-created with p_band / p_band_mag_min / p_band_mag_max /
--                    p_band_snr_min / p_band_snr_max and the 'band_mag' /
--                    'band_snr' sorts: get_filtered_spectra_paginated,
--                    get_filtered_objects_paginated, get_filtered_object_ids,
--                    get_adjacent_objects, get_csv_export_spectra,
--                    get_csv_export_objects
--   views.sql     -- mv_filter_options gains photometry_bands (a matview, which
--                    migra does not track, so it is dropped and recreated here)
--
-- The RPCs gain DEFAULTed parameters only (DROP + CREATE because the signature
-- changes), so the web deploy that lands with this migration keeps working
-- against either version during the minutes the two are not yet both live: the
-- client sends the p_band* parameters only when a band filter is set.
...
-- =============================================================================
-- 1. object_photometry_bands (tables.sql)
-- =============================================================================
...
-- =============================================================================
-- 2. sync_object_photometry_bands (triggers.sql)
-- =============================================================================
...
-- =============================================================================
-- 5. Backfill -- the trigger's own SELECT over every existing cross-match row,
--    so photometry deployed before this migration is filterable immediately.
--    ON CONFLICT DO NOTHING keeps it re-runnable; the table is empty here.
-- =============================================================================
...
CROSS JOIN LATERAL jsonb_each(COALESCE(p.photometry -> 'bands', '{}'::jsonb)) AS e
WHERE jsonb_typeof(e.value) = 'object'
ON CONFLICT (photometry_id, band) DO NOTHING;

ANALYZE public.object_photometry_bands;


-- =============================================================================
-- 6. Band filter helpers (functions.sql)
-- =============================================================================
...
-- =============================================================================
-- 7. The six filter-contract RPCs, re-created with the band parameters
--    (functions.sql). Bare DROPs: they remove every overload, so the added
--    DEFAULTed parameters cannot leave an ambiguous pair behind for PostgREST.
-- =============================================================================
...
-- =============================================================================
-- 8. mv_filter_options gains photometry_bands (views.sql). Matviews are not
--    tracked by the diff engine, so this is the hand-authored half AGENTS.md
--    calls for. Dropped and recreated WITH DATA (after the backfill above, so
--    the band list is populated straight away), then its UNIQUE index is
--    restored -- refresh_filter_options() uses REFRESH ... CONCURRENTLY and
--    needs it.
-- =============================================================================
