...
drop function if exists "public"."count_distinct_inspected_targets"(p_user_id uuid);

drop function if exists "public"."get_adjacent_targets"(p_current_target_id text, p_program_slugs text[], p_filter_programs text[], p_fields text[], p_gratings text[], p_gratings_mode text, p_observations text[], p_redshift_quality integer[], p_redshift_min double precision, p_redshift_max double precision, p_max_snr_min double precision, p_max_snr_max double precision, p_max_exposure_time_min double precision, p_max_exposure_time_max double precision, p_spectral_features integer, p_dq_flags integer, p_spectral_features_include_any integer, p_spectral_features_include_all integer, p_spectral_features_exclude integer, p_dq_flags_include_any integer, p_dq_flags_include_all integer, p_dq_flags_exclude integer, p_list_ids integer[], p_search text, p_inspected_only boolean, p_has_photometry boolean, p_comment_search text, p_comment_search_scope text, p_comment_user_id uuid, p_coord_ra double precision, p_coord_dec double precision, p_radius_degrees double precision, p_sort_column text, p_sort_direction text);

drop function if exists "public"."get_csv_export"(p_program_slugs text[], p_filter_programs text[], p_fields text[], p_gratings text[], p_gratings_mode text, p_observations text[], p_redshift_quality integer[], p_redshift_min double precision, p_redshift_max double precision, p_max_snr_min double precision, p_max_snr_max double precision, p_max_exposure_time_min double precision, p_max_exposure_time_max double precision, p_spectral_features_include_any integer, p_spectral_features_include_all integer, p_spectral_features_exclude integer, p_dq_flags_include_any integer, p_dq_flags_include_all integer, p_dq_flags_exclude integer, p_list_ids integer[], p_search text, p_inspected_only boolean, p_has_photometry boolean, p_comment_search text, p_comment_search_scope text, p_comment_user_id uuid, p_coord_ra double precision, p_coord_dec double precision, p_radius_degrees double precision, p_sort_column text, p_sort_direction text);
...
drop function if exists "public"."get_targets_for_sync"(p_program_slugs text[], p_user_id uuid, p_updated_since timestamp without time zone, p_limit integer, p_offset integer);

drop function if exists "public"."log_flag_changes"();

drop function if exists "public"."propagate_crossmatch_inspection"(p_target_id integer, p_radius_arcsec double precision, p_redshift_tolerance double precision);

drop view if exists "public"."target_flag_summary";

drop view if exists "public"."targets_with_flags";

drop function if exists "public"."update_object_best_redshift"();
...
alter table "public"."flag_audit_log" add column "object_id" integer;

alter table "public"."flag_audit_log" add column "spectrum_id" integer;

alter table "public"."flag_audit_log" alter column "target_id" drop not null;

CREATE INDEX idx_audit_object ON public.flag_audit_log USING btree (object_id) WHERE (object_id IS NOT NULL);

CREATE INDEX idx_audit_spectrum ON public.flag_audit_log USING btree (spectrum_id) WHERE (spectrum_id IS NOT NULL);

CREATE INDEX idx_audit_target ON public.flag_audit_log USING btree (target_id) WHERE (target_id IS NOT NULL);

alter table "public"."flag_audit_log" add constraint "flag_audit_log_subject_check" CHECK ((((((target_id IS NOT NULL))::integer + ((object_id IS NOT NULL))::integer) + ((spectrum_id IS NOT NULL))::integer) = 1)) not valid;

alter table "public"."flag_audit_log" validate constraint "flag_audit_log_subject_check";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.bump_object_version()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.get_csv_export_spectra(p_program_slugs text[], p_filter_programs text[] DEFAULT NULL::text[], p_fields text[] DEFAULT NULL::text[], p_gratings text[] DEFAULT NULL::text[], p_gratings_mode text DEFAULT 'any'::text, p_observations text[] DEFAULT NULL::text[], p_redshift_quality integer[] DEFAULT NULL::integer[], p_redshift_min double precision DEFAULT NULL::double precision, p_redshift_max double precision DEFAULT NULL::double precision, p_max_snr_min double precision DEFAULT NULL::double precision, p_max_snr_max double precision DEFAULT NULL::double precision, p_max_exposure_time_min double precision DEFAULT NULL::double precision, p_max_exposure_time_max double precision DEFAULT NULL::double precision, p_dq_flags_include_any integer DEFAULT NULL::integer, p_dq_flags_include_all integer DEFAULT NULL::integer, p_dq_flags_exclude integer DEFAULT NULL::integer, p_list_ids integer[] DEFAULT NULL::integer[], p_search text DEFAULT NULL::text, p_inspected_only boolean DEFAULT NULL::boolean, p_has_photometry boolean DEFAULT NULL::boolean, p_comment_search text DEFAULT NULL::text, p_comment_search_scope text DEFAULT NULL::text, p_comment_user_id uuid DEFAULT NULL::uuid, p_coord_ra double precision DEFAULT NULL::double precision, p_coord_dec double precision DEFAULT NULL::double precision, p_radius_degrees double precision DEFAULT NULL::double precision, p_sort_column text DEFAULT 'target_id'::text, p_sort_direction text DEFAULT 'asc'::text)
 RETURNS TABLE(target_id text, grating text, field text, ra double precision, "dec" double precision, redshift numeric, redshift_quality integer, redshift_auto double precision, signal_to_noise double precision, exposure_time double precision, fits_path text, program_slug text, program_name text, last_inspected_at timestamp with time zone, last_inspected_by text, distance double precision, dq_flags integer, lists text)
 LANGUAGE plpgsql
 STABLE
...
CREATE OR REPLACE FUNCTION public.log_spectrum_dq_changes()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
with check ((((target_id IS NOT NULL) AND (target_id IN ( SELECT t.id
   FROM public.targets t
  WHERE (t.program_slug = ANY (public.accessible_program_slugs()))))) OR ((object_id IS NOT NULL) AND (object_id IN ( SELECT o.id
   FROM public.objects o
  WHERE (o.programs && public.accessible_program_slugs())))) OR ((spectrum_id IS NOT NULL) AND (spectrum_id IN ( SELECT s.id
   FROM (public.spectra s
...
CREATE TRIGGER bump_object_version_trigger BEFORE UPDATE ON public.objects FOR EACH ROW EXECUTE FUNCTION public.bump_object_version();

CREATE TRIGGER track_object_inspection_trigger BEFORE UPDATE OF redshift_quality ON public.objects FOR EACH ROW EXECUTE FUNCTION public.log_object_inspection_changes();

CREATE TRIGGER track_spectrum_dq_changes AFTER UPDATE OF dq_flags ON public.spectra FOR EACH ROW EXECUTE FUNCTION public.log_spectrum_dq_changes();




-- ===========================================================================
-- Manually appended (Phase D.2): migra does not track grants on materialized
-- views, comments, or function executes. We re-grant for the (re)created
-- objects below to match the schema files.
-- ===========================================================================

-- Materialized views — recreated above with WITH DATA but no grants.
...
GRANT SELECT ON TABLE public.mv_programs_overview TO authenticated;

-- spectrum_flag_summary view (replaces target_flag_summary)
...
-- New / dropped+recreated functions
...
GRANT EXECUTE ON FUNCTION public.get_csv_export_spectra(
...
GRANT EXECUTE ON FUNCTION public.get_csv_export_objects(
...
GRANT EXECUTE ON FUNCTION public.get_filtered_spectra_paginated(
  TEXT[], TEXT[], TEXT[], TEXT[], TEXT, TEXT[], INTEGER[],
  DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION,
  DOUBLE PRECISION, DOUBLE PRECISION,
  INTEGER, INTEGER, INTEGER,
