...
# Emission-line fit product (cfpipe nirspec linefit): per-spectrum line fluxes at
# the inspected redshift, written next to the spectrum. New product, so no legacy
# key: reserved canonical form. Its '_lines.fits' suffix must be dispatched before
# the bare-'.fits' spectrum-exposure fallback in bijection._nirspec_obs_product.
_register(ProductSpec(
    name="nirspec_lines", instrument=NS, tree="products", bucket="data",
    lifecycle=LC.CLOUD_PRODUCT, scope_keys=("obs",), subdir=_nirspec_obs_dir,
...
# Inspected redshifts pulled from the portal (objects.redshift / redshift_quality),
# read by cfpipe nirspec linefit. Regenerated by `campfire pull`; DB-authoritative.
_register(ProductSpec(
    name="nirspec_redshifts", instrument=NS, tree="reference", bucket="data",
    lifecycle=LC.USER_STATE, scope_keys=("obs",), subdir=_nirspec_obs_dir,
    suffix="redshifts.toml",
    filename=lambda s: "redshifts.toml", legacy_prefix=None,
))
_register(ProductSpec(
