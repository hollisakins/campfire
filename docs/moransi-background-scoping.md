...
| **3b — Scientific A/B run** | ⏳ needs data | Run the harness on representative real tiles **stratified by channel (SW/LW), pixel scale, depth/field**; require per-regime acceptance; record decision. **No default change.** | (data + cluster) |
