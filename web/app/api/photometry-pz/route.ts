...
/**
 * GET /api/photometry-pz?object_id=<object_id>&field=<field>
 *
...
export async function GET(request: NextRequest) {
  ...
  if (!objectId || !field) {
    return NextResponse.json(
      { error: 'Missing object_id or field parameter' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    // Construct P(z) sidecar path in R2
    const pzPath = `photometry/${field}/${objectId}_pz.json`;
