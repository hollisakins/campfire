...
![Before and after comparison on an Abell 2744 exposure with a bright elliptical: the conventional row/column-median treatment leaves bright blocky patches and streaks around the galaxy, while the CAMPFIRE result is clean.](/docs/reduction/nircam-gp-vs-median.png)

*Why the Gaussian-process treatment matters, demonstrated on a public UNCOVER exposure (`jw02561006002_07201_00001`, NRCB1, F200W) with a bright elliptical whose envelope spans amplifier rows; all panels are shown at native pixel scale and a common ±1.3σ stretch (dotted lines mark amplifier boundaries). Left: the input frame. Middle: after a plain per-amp-row and per-column median subtraction with no conditioning (the conventional treatment) — the galaxy's envelope biases the row and column estimates it touches, leaving over-subtracted blocky patches and streaks around the galaxy with hard edges at the amplifier boundaries, coherent over hundreds of pixels, and eating into the envelope itself. Right: the CAMPFIRE default (GP + conditioning detrend + deep mask) removes the striping without these artifacts and preserves the envelope.*
