...
export function objectTokenMessage(key: string, hash: string, exp: number, upstream: string): string {
  return [TOKEN_VERSION, key, hash, String(exp), upstream].join('\n');
}
...
export function buildFrontUrl(
  ...
  return (
    ...
    `?h=${encodeURIComponent(hash)}&e=${exp}&t=${token}&u=${encodeURIComponent(upstream)}`
  );
}
  ...
  await Promise.all(
    resolved.map(async (o, i) => {
      ...
      try {
        ...
        const token = await hmacBase64Url(objectTokenMessage(o.key, o.contentHash, exp, url), secret);
        out.set(keys[i], buildFrontUrl(base, o.key, o.contentHash, exp, url, token));
      } catch (err) {
