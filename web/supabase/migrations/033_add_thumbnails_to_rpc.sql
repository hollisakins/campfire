-- Migration 033: Add thumbnail support to get_filtered_objects_paginated
-- Adds p_include_thumbnails parameter to optionally include pre-generated SVG thumbnails
-- in the spectra JSONB array. This eliminates ~100 HTTP requests per table page.
--
-- When p_include_thumbnails is FALSE (default), thumbnails are excluded to keep
-- Python API responses lean. The web app sets this to TRUE for table views.
...
CREATE OR REPLACE FUNCTION get_filtered_objects_paginated(
  ...
  -- NEW: Include thumbnail SVGs in spectra (for web table views)
