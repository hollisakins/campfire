...
L.Control.Settings = L.Control.extend({
    ...
    makeRange: function (text, min, max, defaultValue, step) {
        ...
        rangeText += `<input class="settings-range" ondblclick="settingsControl.resetRange('${text}', '${defaultValue}')" oninput="settingsControl.updateCSS();settingsControl.moveBadge('${text}')" onfocusout="settingsControl.hideBadge('${text}')" onmouseup="settingsControl.hideBadge('${text}')" type="range" min="${min}" max="${max}" value="${defaultValue}" step="${step}" class="range" id="${text}" />`;
